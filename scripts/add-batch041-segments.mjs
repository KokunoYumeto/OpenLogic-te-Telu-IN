...
if (existing.some(isBatchRow) && !refresh) throw new Error('Batch 041 segment rows already exist; pass --refresh to rebuild their mechanical hashes and spans');
...
const canonByUnit = {
  'OLP-0356': ['TE-P027', 'TE-P029'],
  'OLP-0357': ['TE-P003', 'TE-P027', 'TE-P029'],
  'OLP-0358': ['TE-P003', 'TE-P027', 'TE-P029', 'TE-P032'],
  'OLP-0359': ['TE-P027', 'TE-P029'],
  'OLP-0360': ['TE-P008', 'TE-P011', 'TE-P027', 'TE-P029'],
};
...
for (const unit of manifest) {
  ...
  sourceBlocks.forEach((sourceBlock, index) => {
    ...
    const structural = /^(?:%[^\n]*(?:\n%[^\n]*)*|\\documentclass\[[^\]]*\]\{[^{}]+\}|\\begin\{document\}|\\olfileid(?:\{[^{}]*\}){3}|\\olimport(?:\[[^\]]+\])?\{[^{}]+\}|\\OLEnd(?:Chapter|Part)Hook|\\end\{document\})$/u.test(trimmed);
    ...
    rows.push({
      ...
      consultation_phase: structural
        ...
        : 'Contemporaneous Batch 041 choice ' + segmentId + '; exact source and Telugu target were read in full after visual consultation of the listed native pages and review of TE-T078 and TE-T080.',
      evidence_limit: structural
        ...
        : 'The inspected native pages support mathematical explanation, induction and proof prose, set and function usage, formal terms and variables, and scope/binding vocabulary. They do not directly attest lambda abstraction, its parameter, unique term formation, combinators or the precise free-variable equations. Frozen formation rules, the corrected scope definition and TE-T078/TE-T080 control those specialized senses. OLTELAMSYN-001--004 record local source defects, not language-canon claims.',
    });
  });
}
