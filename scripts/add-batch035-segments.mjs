...
if (existing.some(isBatchRow) && !refresh) throw new Error('Batch 035 segment rows already exist; pass --refresh to rebuild their mechanical hashes and spans');
  ...
  sourceBlocks.forEach((sourceBlock, index) => {
    ...
    rows.push({
      ...
      consultation_phase: structural
        ...
        : `Contemporaneous Batch 035 choice ${segmentId}; exact source and Telugu target were read in full and checked against the listed native passages and TE-T034, TE-T047, TE-T058, TE-T061, TE-T066, TE-T067 and TE-T070--TE-T074 as applicable.`,
      evidence_limit: structural
        ...
        : 'The cited Andhra Pradesh, distinct school-mathematics, and pre-bifurcation Telangana-hosted native passages support Telugu number, set, relation, function, truth-value, formula, sentence, first-order, domain, derivation, inference, consistency and proof register. They do not directly attest fixed points, diagonalization, arithmetized proof predicates, Rosser provability, reflection, Loeb derivability or Tarski undefinability. The frozen definitions and proofs plus TE-T074 control those specialized senses. OLTEINP-001--005 are source-audit decisions, not language-canon claims.',
    });
  });
}
