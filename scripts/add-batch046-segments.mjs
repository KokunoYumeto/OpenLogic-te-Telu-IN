...
if (existing.some(row => row.unit_id === unitId) && !refresh) {
  throw new Error('Batch 046 segment rows already exist; use --refresh after target revision');
}
const retained = existing.filter(row => row.unit_id !== unitId);
const unit = jsonl(manifestPath).find(row => row.unit_id === unitId);
if (!unit || unit.order !== 364) throw new Error('Manifest cursor mismatch');
...
const rows = sourceBlocks.map((sourceBlock, index) => {
  ...
  return {
    ...
    consultation_phase: structural
      ...
      : 'OLP-0364 అనువాద సమయంలో ' + segmentId + ' మూలం, తెలుగు లక్ష్యాన్ని ఎదురెదురుగా చదివి; సూచించిన స్థానిక పేజీలను ప్రత్యక్షంగా చూసి TE-T082, TE-T086తో పోల్చాం.',
    evidence_limit: structural
      ? 'వాక్యేతర TeX నిర్మాణం లేదా మూల గుర్తింపు వ్యాఖ్య; పాఠక గద్యం మానలేదు.'
      : 'పేజీలు సమితి, సంబంధం, చరం, పరిధి, బద్ధ చరం, నిర్వచన గద్యానికి ఆధారం. ఆల్ఫా-తుల్యతా భాగవర్గం, ప్రతినిధి ఎంపిక స్వాతంత్ర్యం, వర్గాలపైకి పటాన్ని దింపడానికి ప్రత్యక్ష సాక్ష్యం కావు; ప్రత్యేక అర్థానికి మూల నిర్వచనలు, OLTELAMTR-001–002 పరిశీలనలే ఆధారం. పూర్వ నిరూపణ ఖాళీలు ఇంకా తెరిచి ఉన్నాయి.',
  };
});
