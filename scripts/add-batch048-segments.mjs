...
if (existing.some(row => row.unit_id === unitId) && !refresh) {
  throw new Error('Batch 048 segment rows already exist; use --refresh after target revision');
}
const retained = existing.filter(row => row.unit_id !== unitId);
const unit = jsonl(manifestPath).find(row => row.unit_id === unitId);
if (!unit || unit.order !== 366) throw new Error('Manifest cursor mismatch');
...
if (sourceBlocks.length !== 18 || targetBlocks.length !== sourceBlocks.length) {
...
const passagesForBlock = [
  [], [], [], ['TE-P027', 'TE-P032'],
  ['TE-P008', 'TE-P027', 'TE-P029'],
  ['TE-P027', 'TE-P029', 'TE-P032'],
  ['TE-P027', 'TE-P032'],
  [],
  ['TE-P027', 'TE-P029', 'TE-P032'],
  ['TE-P008', 'TE-P027'],
  ['TE-P008', 'TE-P027', 'TE-P029', 'TE-P032'],
  ['TE-P008', 'TE-P032'],
  ['TE-P032'],
  ['TE-P032'],
  ['TE-P029', 'TE-P032'],
...
const structural = new Set([0, 1, 2, 7, 17]);
const rows = sourceBlocks.map((sourceBlock, index) => {
  ...
  const canonPassages = passagesForBlock[index].map(id => {
    const passage = passageCatalog.get(id);
    if (!passage) throw new Error('Missing inspected canon passage ' + id);
    return passage;
  });
  if (structural.has(index) !== (canonPassages.length === 0)) throw new Error('Block classification mismatch ' + segmentId);
  ...
  return {
    ...
    classification: structural.has(index) ? 'preserved_metadata_or_structural_segment' : 'translated_linguistic_segment',
    canon_passages: canonPassages,
    source_corrections: sourceCorrections,
    consultation_phase: structural.has(index)
      ? 'not_applicable_nonlinguistic'
      : 'OLP-0366 అనువాద సమయంలో ' + segmentId + ' మూలం, తెలుగు లక్ష్యాన్ని ఎదురెదురుగా చదివి; సూచించిన స్థానిక పేజీలను ప్రత్యక్షంగా చూసి TE-T046, TE-T078, TE-T088తో పోల్చాం.',
    evidence_limit: structural.has(index)
      ? 'వాక్యేతర TeX నిర్మాణం లేదా ముగింపు మాత్రమే; పాఠక శీర్షిక, గద్యం ఇందులో దాచలేదు.'
      : 'పేజీలు ప్రమేయం, పదం, చరం, బద్ధ/స్వేచ్ఛా చరం, సిద్ధాంత-నిరూపణ శైలికి మాత్రమే ఆధారం. ఏటా-సంకోచనం, బీటా-ఏటా తగ్గింపు, లాంబ్డా పదాల విస్తారతా నియమం లేదా సమానత్వ నిరూపణకు ప్రత్యక్ష సాక్ష్యం కావు; ప్రత్యేక అర్థానికి స్థిర మూల నియమం, ప్రకటిత సవరణలే ఆధారం.',
  };
});
