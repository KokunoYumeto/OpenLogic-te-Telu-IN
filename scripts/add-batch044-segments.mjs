...
if (existing.some(row => row.unit_id === unitId) && !refresh) {
  throw new Error('Batch 044 segment rows already exist; use --refresh after target revision');
}
const retained = existing.filter(row => row.unit_id !== unitId);
const unit = jsonl(manifestPath).find(row => row.unit_id === unitId);
if (!unit || unit.order !== 363) throw new Error('Manifest cursor mismatch');
...
if (sourceBlocks.length !== 16 || targetBlocks.length !== sourceBlocks.length) {
...
const rows = sourceBlocks.map((sourceBlock, index) => {
  ...
  return {
    ...
    consultation_phase: structural
      ...
      : 'OLP-0363 అనువాద సమయంలో ' + segmentId + ' మూలం, తెలుగు లక్ష్యాన్ని ఎదురెదురుగా చదివి; సూచించిన స్థానిక పేజీలను ప్రత్యక్షంగా చూసి TE-T078, TE-T082, TE-T083తో పోల్చాం.',
    evidence_limit: structural
      ? 'వాక్యేతర TeX నిర్మాణం లేదా మూల వ్యాఖ్య; పాఠక గద్యం మానలేదు.'
      : 'పేజీలు పదం, చరం, పరిధి, బంధనం, సమితి, ప్రమేయం, నిర్వచనం, ఆగమన వివరణకు ఆధారం. డి బ్రూయిన్ సూచికలు, తొలి ఘటన స్థాన నిర్ణయం, తిరుగు పటం పాక్షికతకు ప్రత్యక్ష సాక్ష్యం కావు; వీటికి మూల సమీకరణాలు, OLTELAMDEB-001–003 పరిశీలనలు ఆధారం.',
  };
});
