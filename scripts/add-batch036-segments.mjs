...
if (existing.some(isBatchRow) && !refresh) throw new Error('Batch 036 segment rows already exist; pass --refresh to rebuild their mechanical hashes and spans');
  ...
  sourceBlocks.forEach((sourceBlock, index) => {
    ...
    rows.push({
      ...
      consultation_phase: structural
        ...
        : 'Contemporaneous Batch 036 choice ' + segmentId + '; exact source and Telugu target were read in full and checked against the listed native passages and TE-T034, TE-T047, TE-T051, TE-T058, TE-T061 and TE-T070--TE-T075 as applicable.',
      evidence_limit: structural
        ...
        : 'The cited Andhra Pradesh, distinct school-mathematics, and pre-bifurcation Telangana-hosted native passages support Telugu number, set, relation, function, truth-value, formula, sentence, first-order, domain, derivation, inference and proof register. They do not directly attest full standard second-order semantics, relation/function variables, expressive power, transitive closure or Dedekind infinitude. The frozen definitions and proofs plus TE-T051 and TE-T075 control those specialized senses. OLTESOLSYN-001--006 are source-audit decisions, not language-canon claims.',
    });
  });
}
