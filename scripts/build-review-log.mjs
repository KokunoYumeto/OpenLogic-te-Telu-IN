...
const completion='partial_260_of_722_draft_units';
...
const phase='Evidence reconstruction through 2026-09-19 from the current primary TERM_DECISIONS, aligned segment ledger, canonical-passage records and exact source/target bytes; earlier records are not represented as contemporaneous pre-draft notes, while TE-T064--TE-T068 record the Batch 025--Batch 029 consultations performed during reconciliation.';
...
const correctionRecords=corrections.map(c=>{
 ...
 return {review_id:'REV-'+c.finding_id,record_type:'source_correction_decision',scope_completion:completion,language:'Telugu',script:'Telu',locale:'te-Telu-IN',finding_id:c.finding_id,classification:c.classification,...(c.qualification?{qualification:c.qualification}:{}),confidence:'high_mathematical_repair_moderate_disclosure_wording',review_priority:'medium',expert_review_status:qualified?'historical_error_classification_rejected_equivalent_notation_qa_passed_disclosure_wording_open_for_optional_review_no_hold':'mathematical_correction_qa_passed_disclosure_wording_open_for_optional_review_no_hold',implementation_locations:locations,actual_authorities_checked:[{audit_id:c.audit_id,audit_review_sha256:c.audit_review_sha256,audit_findings_sha256:c.audit_findings_sha256},...(qualified?[{qualification_review_sha256:c.qualification.consolidation_review_sha256,disposition:c.qualification.disposition,review_path:c.qualification.review_path}]:[]),{source_revision:'9620cc73f9c8e0ad003c514a5d3748f29611c4c0',source_path:c.source_path,source_sha256:c.source_sha256}],not_checked_or_not_found:[qualified?'No independent human subject expert has reviewed the Telugu qualification wording yet; the equivalent notation and false-positive disposition were checked by the consolidation review and correction-aware structural QA.':'No independent human subject expert has reviewed the Telugu disclosure wording yet; the mathematical treatment was checked by the recorded source audit and correction-aware structural QA.'],rationale:c.body_treatment,alternatives_considered_or_recorded:qualified?['Retain the valid nested cardinality construction verbatim (viable, but the explicit pair is clearer in the target).','Present the two equivalent explicit comparisons and disclose the rejected historical classification (chosen).']:['Translate the defective source claim verbatim (rejected because it would knowingly reproduce the defect).','Apply the recorded minimal mathematical repair and disclose it adjacent to the translated claim (chosen).'],uncertainty:qualified?'Low for mathematical equivalence and the rejected-false-positive disposition; optional review remains useful for the clarity of its Telugu qualification.':'Low for the recorded mathematical repair; optional review remains useful for the clarity of its Telugu disclosure.',rationale_phase:qualified?'Historical audit record retained and qualified by the 2026-09-19 consolidation review, followed by correction-aware batch QA.':'Contemporaneous application of the recorded source audit, followed by correction-aware batch QA.',precise_review_questions:[question],translation_hold:false,status:c.status};
});
...
const priorityMd=['# Priority optional expert-review view','',`Scope: **partial — 260 of 722 draft units**. This view selects ${priorityRecords.length} of ${records.length} open decisions whose nomenclature is highly provisional or whose correction disclosure merits a human clarity check. It creates no translation hold.`,'','Final printed/PDF pages remain pending coherent-reader pagination; exact unit, section, file and line locators are supplied.',''];
