...
if (existing.some(row => row.unit_id === unitId) && !refresh) {
  throw new Error('Batch 042 segment rows already exist; use --refresh after target revision');
}
const retained = existing.filter(row => row.unit_id !== unitId);
const unit = jsonl(manifestPath).find(row => row.unit_id === unitId);
if (!unit || unit.order !== 361) throw new Error('Manifest cursor mismatch');
...
if (sourceBlocks.length !== 22 || targetBlocks.length !== sourceBlocks.length) {
...
const passageIds = ['TE-P003', 'TE-P008', 'TE-P027', 'TE-P029', 'TE-P032'];
const passages = passageIds.map(id => {
  const passage = passageCatalog.get(id);
  if (!passage) throw new Error('Missing consulted canon passage ' + id);
  return passage;
});
const rows = sourceBlocks.map((sourceBlock, index) => {
  ...
  return {
    ...
    consultation_phase: structural
      ...
      : 'OLP-0361 అనువాద సమయంలో ' + segmentId + ' మూలం, తెలుగు లక్ష్యాన్ని ఎదురెదురుగా చదివి; సూచించిన స్థానిక పేజీలను ప్రత్యక్షంగా చూసి TE-T078, TE-T080, TE-T081తో పోల్చాం.',
    evidence_limit: structural
      ? 'వాక్యేతర TeX నిర్మాణం లేదా మూల వ్యాఖ్య; పాఠక గద్యం మానలేదు.'
      : 'పేజీలు పదాలు, చరాలు, బంధనం, పరిధి, సమితి, ప్రమేయం, ఆగమన నిరూపణ భాషకు ఆధారం. లాంబ్డా ప్రతిస్థాపన యొక్క పాక్షిక నియమం లేదా చరపట్టివేత నివారణకు ప్రత్యక్ష సాక్ష్యం కావు; ఆ ప్రత్యేక భావాలకు మూల నిర్వచనమూ OLTELAMSUB-001–007 పరిశీలనలూ ఆధారం.',
  };
});
const output = [...retained, ...rows].map(JSON.stringify).join('\n') + '\n';
...
process.stdout.write(JSON.stringify({
  ...
  rows: rows.length,
  linguistic,
  structural: rows.length - linguistic,
}) + '\n');
