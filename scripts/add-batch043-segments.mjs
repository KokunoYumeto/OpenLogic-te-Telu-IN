...
if (existing.some(row => row.unit_id === unitId) && !refresh) {
  throw new Error('Batch 043 segment rows already exist; use --refresh after target revision');
}
const retained = existing.filter(row => row.unit_id !== unitId);
const unit = jsonl(manifestPath).find(row => row.unit_id === unitId);
if (!unit || unit.order !== 362) throw new Error('Manifest cursor mismatch');
...
if (sourceBlocks.length !== 39 || targetBlocks.length !== sourceBlocks.length) {
...
const rows = sourceBlocks.map((sourceBlock, index) => {
  ...
  return {
    ...
    consultation_phase: structural
      ...
      : 'OLP-0362 అనువాద సమయంలో ' + segmentId + ' మూలం, తెలుగు లక్ష్యాన్ని ఎదురెదురుగా చదివి; సూచించిన స్థానిక పేజీలను ప్రత్యక్షంగా చూసి TE-T078, TE-T081, TE-T082తో పోల్చాం.',
    evidence_limit: structural
      ? 'వాక్యేతర TeX నిర్మాణం లేదా మూల వ్యాఖ్య; పాఠక గద్యం మానలేదు.'
      : 'పేజీలు పదం, చరం, బంధనం, పరిధి, సంబంధం, ప్రమేయం, నిర్వచనం, ఆగమన నిరూపణ భాషకు ఆధారం. ఆల్ఫా-పరివర్తనం లేదా ప్రతిస్థాపన ఏకత్వ నిరూపణకు ప్రత్యక్ష సాక్ష్యం కావు; ప్రత్యేక భావానికి మూల నియమాలే, లోపాల గుర్తింపుకు OLTELAMALP-001–007 పరిశీలనలే ఆధారం.',
  };
});
...
process.stdout.write(JSON.stringify({
  mode: refresh ? 'refresh' : 'append', unit: unitId, rows: rows.length,
